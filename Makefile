...
# Compiler flags
COMMON_FLAGS = -Wall -Wextra -Wshadow -Wformat=2 -Wpedantic -MMD -MP # Common flags for warnings, format checking, and dependency generation
CFLAGS = -O3 -flto $(COMMON_FLAGS) -g -Wno-format-nonliteral # Optimization level 3, link-time optimization, debug information, suppress “not a string literal” warnings.

# Linker flags
LDFLAGS = -lpthread -lssl -lcrypto # Linker flags to include necessary libraries (pthread, OpenSSL)
...
clean:
	rm -f $(OBJ_SRC_DIR)/*.o $(OBJ_SRC_DIR)/*.d $(OBJ_TEST_DIR)/*.o $(OBJ_TEST_DIR)/*.d $(TARGET) $(OBJ_TEST_DIR)/test_runner $(OUTPUT_DIR)/*
